...
M4（普通输入、菜单与设置）代码迁移已完成：`src/ui/fields.rs` 以 `TextFieldState` 保持业务真值、Kit `InputState` / `TextareaState` 承担渲染与编辑，`ensure_kit_fields` 自动覆盖 `DEDICATED_FIELDS` 中除冲突草稿外的全部静态字段；工作流动态字段和冲突草稿按计划留到 M6。多行普通 Enter 只换行，Ctrl/Cmd+Enter 由根层 `TextSubmit` 在 Kit 写入前提交。动态重绑快捷键只清除 Khaslana 自身 action，保留 Kit 的按钮、输入和菜单键位。项目级通用按钮使用 Kit 基础按钮；远端操作与标签推送的简单选择器使用 Kit `DropdownMenu`。真实前台键盘/焦点回归仍需按 M7 验收矩阵复验；向窗口注入按键要用 `PostMessage`，`SendMessage` 的同步重入会被 gpui 丢弃（WM_CHAR 两者皆可，WM_KEYDOWN 只有 `PostMessage` 生效）。
...
- `src/ui/`：前端设计系统适配层。`theme.rs` 定义主题感知语义 token；`components.rs` 封装项目级 UI helper，通用文字/图标/分段按钮以 Kit 基础按钮承载焦点、Tab 与 Enter/Space；`kit_theme.rs` 把 Khaslana token 映射到 Kit Theme。悬浮工作台使用 `WB_*` token、`floating_panel` / `navigator_panel` / `settings_card` 与统一阴影；表面色经 `rgb()` 时必须为不带 alpha 的 6 位值，透明色只用 `rgba()`。`fields.rs` 是 M4 Kit 输入适配层：`TextFieldState` 保持业务真值，Kit `InputState` / `TextareaState` 负责渲染、选区、IME 与剪贴板；`ensure_kit_fields` 自动迁移 `DEDICATED_FIELDS` 中除 `ConflictEditor` 外的全部静态字段，工作流动态字段与冲突草稿留到 M6。用户编辑经 `InputEvent::Change` 写回真值，程序回填下一帧推给 Kit；`focused_field` 必须先让位给 Kit 字段。多行 `PressEnter` 不提交，Ctrl/Cmd+Enter 由 Input 上的 `TextSubmit` 覆盖绑定在 Kit 插入换行前处理。
...
- **键盘交互（Kit 标准）**：普通控件参与 Tab/Shift+Tab；按钮支持 Enter/Space；菜单支持方向键、Enter 与 Esc；可取消浮层支持 Esc 并恢复到稳定焦点。文本框保留编辑、IME 与业务提交语义：多行普通 Enter 换行，Ctrl/Cmd+Enter 提交；单行 Enter 提交所在表单。快捷键录制、Ctrl+P 面板、应用级快捷键及变更列表 Shift/Ctrl+点击多选保持原有业务守卫。`register_all_key_bindings` 动态刷新时必须保留 Kit 已注册的组件键位，只移除 `text_input::` / `app_action::` 命名空间的应用绑定。
