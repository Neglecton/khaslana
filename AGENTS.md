...
- `src/code_index/queries.rs`：索引查询 API 层（Phase 2 查询面，参照 codebase-memory-mcp 工具语义）。`find_symbol_candidates` 精确名/qualified_name 查找；`trace_calls` CALLS 边 BFS（起点=同名词根联合遍历，visited 防环，max_nodes=100；风险标签移植 cbm_hop_to_risk：hop 1→CRITICAL/2→HIGH/3→MEDIUM/≥4→LOW；同名可调用多候选返回 Ambiguous 让调用方用 qualified_name 重查）；`symbol_detail` 定义信息 + 一跳 callers/callees + 源码片段（rel_path 读文件钳 200 行/1MB）；`index_overview` label/边类型分布 + 扩展名语言聚合 + 顶层目录符号密度 + fan-in Top10 热点；`changed_files_via_git`（git2 实现，替代参考项目 shell git 三命令：默认 statuses 三态并集=未暂存+已暂存+未跟踪，传 base_branch 走 merge-base 三点 diff）；`impacted_symbols`/`impacted_symbols_for_files` 变更文件→文件内定义符号→CALLS 入边展开调用方（Phase 4 影响分析雏形）。图遍历基于 store.load_graph() 单次载全图（Phase 1 规模毫秒级；超大仓库优化点=SQL 邻接查询）。
- `src/code_index/mcp.rs`：内嵌 MCP stdio 服务器（lib crate，可单测），由 `main()` 顶部 `khaslana mcp <仓库路径>` 子命令分支调用（先于 GUI 启动；不初始化 stdout 版 tracing 防污染协议流）。协议逐条对照参考项目 mcp.c：换行分隔 JSON 帧 + Content-Length 兼容（10MB 上限）；initialize 协商 protocolVersion（2025-06-18/2025-03-26/2024-11-05，未知回最新）+ capabilities.tools + serverInfo；notifications 静默、ping→{}、未知 method -32601、解析失败 -32700（id 0）、字符串/数字 id 原样回显；tools/call 结果 = 单行紧凑 JSON text + structuredContent 双通道（非错误且可解析为对象时附加），业务错误 = isError:true + {error, hint} 自纠错提示。7 个工具：search_symbols/get_symbol_detail/trace_path/get_architecture/detect_changes/index_status/refresh_index。单线程串行处理（无锁）；启动索引保障：无库/空库自动全量建、有库增量检查（对齐参考项目 auto_index；MCP 配置本身即显式的 per-仓库意图），进度一律 stderr。核心 `handle_message` 纯函数式，协议行为有单测。
- `src/code_palette_view.rs`：全局符号搜索面板（Ctrl+P，`ShortcutAction::OpenCodeSearch`，默认 ctrl-p 可在快捷键设置改）。settings_center 式 dialog_overlay 居中双栏面板（左 320px 结果列表 / 右详情）：输入即查（每次变更 `code_palette_search_seq`+1 → Short 求 search_symbols(50)，事件带 seq 守卫乱序丢弃）；选中行变化 → `code_palette_detail_seq` 守卫取 `symbol_detail`（直接 callers/callees + 源码片段）；↑↓ 切换/Enter=关闭并 `open_blame_file`（内置追溯视图，最接近「跳到代码」）/Esc 关闭——在面板层 `capture_key_down` 拦截（先于输入框 TextUp/TextDown/Enter 提交），§8 白名单已记录该例外；点击遮罩关闭（overlay on_mouse_down + 面板 stop_propagation）；底部操作行复用 追溯此文件/文件历史/复制绝对路径/打开所在目录 四个现有动作；面板关闭后输入内容保留（重开续用并立即按现值查询）。挂载层级在普通对话框之上、AI 思考弹窗之下；入口监听用根壳层元素级 `.on_action`（区别于其他快捷键的 App::on_action——元素级回调带 Window 可聚焦输入框）。
...
- MCP 服务器（Phase 2 第一步）：`khaslana mcp <仓库路径>` 以无头模式启动内嵌 MCP stdio 服务器（协议对齐 codebase-memory-mcp：initialize 协商/tools/list/tools/call、单行 JSON text + structuredContent 双通道、isError+hint 业务错误约定），向 Claude Code / Cursor / ZCode 等外部 AI 工具暴露 7 个查询工具（search_symbols/get_symbol_detail/trace_path/get_architecture/detect_changes/index_status/refresh_index）。启动时自动建立/增量刷新索引（对齐参考项目 auto_index）。设置中心「代码索引」页提供可一键复制的客户端配置片段（exe 路径 + `mcp <repo>` 参数）。detect_changes 即 Phase 4「Git 变更影响分析」的查询雏形：工作区三态变更 → 受影响符号 → 上游调用方风险分级。
