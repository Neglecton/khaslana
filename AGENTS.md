...
本次重构采用 Kit 标准交互：Tab/Shift+Tab、按钮 Enter/Space、菜单方向键、可取消浮层 Esc 与焦点恢复，并保留可见焦点指示。迁移范围内，此决策覆盖下文旧的普通控件纯鼠标、禁止聚焦和键盘导航规则；现有应用快捷键及录制、Ctrl+P、文本编辑、IME、业务守卫、危险确认与任务生命周期保持原语义。M4 已把项目级通用按钮切到 Kit 基础按钮、两处简单远端选择切到 Kit `DropdownMenu`，并在根层补齐可取消浮层 Esc 收口。
...
开关语义的控件统一为 **Kit `Switch` 滑块**（经 `RepositoryView::toggle_switch` 封装受控回调：Kit Switch 只把请求值交给 `on_change`，由调用方写回状态；启用时不拦截冒泡，故不得在外层容器重复挂同一动作）。「开关 + 文字」行用 `toggle_row`，开关本体与文字区各自可点、互不重叠；开关可 Tab 聚焦并以 Enter/Space 激活。复选框样式的自绘 `toggle_box` 与代码索引页的自绘 `code_index_switch` 均已删除。

窗口外观两项已决（不要反复重提）。**圆角采用样板路线**：窗口背景透明（`WindowBackgroundAppearance::Transparent`）+ 外壳自绘 24px 圆角（`RADIUS_WINDOW`），并由 `chrome_view::apply_window_chrome` 在窗口创建后与下一帧各执行一次，去掉 `WS_THICKFRAME | WS_CAPTION` 并触发 gpui 重算边框偏移——不去边框则 DWM 会沿顶边画 1px 线（圆角外是透明区，正好从那里透出来），客户区还会比窗口矩形小 16 × 8（最小窗 860 × 520 被放大成 876 × 528，去边框后实测精确 860 × 520、客户区与窗口矩形等大）。**Kit `Root` 必须显式覆盖成透明底**（`Root::new(..).bordered(false).bg(rgba(0x00000000))`）：`Root` 自己会铺 `theme.tokens.background`，不覆盖的话圆角外那一圈会被涂成实色，圆角在屏幕上就“看不见”（实测：覆盖前四角像素是 `#FFFFFF`，覆盖后该处不再有任何绘制）。**只去 caption 保留 thickframe 的方案已实测否决**：客户区仍内缩 14 × 7，圆角卡片会被一圈系统框包着，且顶边那条线来自 thickframe。**去边框的代价是四边系统缩放带一起消失**（gpui 只自己补了顶边），所以 `render_window_resize_bands` 在左/右/下三边与两个下角铺 6px 透明带，按下时用 `start_window_resize` 发 `WM_NCLBUTTONDOWN` + `HTLEFT/HTRIGHT/HTBOTTOM/HTBOTTOMLEFT/HTBOTTOMRIGHT` 把该边交回 `DefWindowProc` 的模态缩放循环（最小尺寸、DPI、贴边仍由系统负责；最大化时整组不渲染；带子挂最外层，弹窗打开时也能缩放）。透明本身可用 `GetWindowLongPtrW(GWL_EXSTYLE)` 复核：`WS_EX_NOREDIRECTIONBITMAP (0x200000)` 为真即走 DWM 直合成（无重定向位图 → 没画到的地方就是桌面）。**圆角裁切只有一层**：gpui 的 `overflow_hidden` 只裁矩形，圆角必须由每个铺满窗口的色块自己画——根背景（`app_shell_surface`）、顶栏上角、`dialog_overlay()` 与操作遮罩都读同一个 `ui_theme::window_radius()`；状态栏因此改为透明底色，让根圆角透出来。**最大化时圆角归零**（`ui_theme::set_window_radius` 在 `render` 里按 `window.is_maximized()` 写入），否则四角会露出桌面。**不为窗口投影做额外工作**：去边框就丢了 DWM 阴影，本条决定不补自绘投影、也不在窗口内留边距，层次全部由窗口内部的面板投影与环境底表达。标题栏**最大化按钮必须能还原**，实现走 `WindowControlArea::Max` 原生命中区（gpui-pre-windows 对 `HTMAXBUTTON` 的单击本身就是最大化↔还原切换），图标按 `window.is_maximized()` 切换，不使用只最大化不还原的 `zoom_window()`；实测最大化后窗口填满屏幕（1920 × 1080，圆角归零）并可再次点击还原。

M3（壳层与首个设置页）已落地：60px 顶栏、仅仓库名的仓库下拉、顶栏搜索入口（开 Ctrl+P）、中文命令（窄档图标 + tooltip）、32×32 紧凑窗口按钮、导航底部设置齿轮、内容区悬浮面板与间隙式分栏拖拽区、主题页与代理页新样板，以及窗口根视图接入 Kit `Root`（M2 遗留缺口）。见 `docs/gpui-kit-refactor-plan.md` §8。

M4（普通输入、菜单与设置）代码迁移已完成：`src/ui/fields.rs` 以 `TextFieldState` 保持业务真值、Kit `InputState` / `TextareaState` 承担渲染与编辑，`ensure_kit_fields` 自动覆盖 `DEDICATED_FIELDS` 中除冲突草稿焦点锚点外的全部静态字段。M6 已收口两处遗留：**工作流动态字段**（`WorkflowInput` / `WorkflowEditor`）迁入 Kit——动态字段不进 `DEDICATED_FIELDS`，宿主由 `reconcile_dynamic_kit_fields` 每帧按 `active_dynamic_field_ids`「多退少补」，事件回调走 `try_field` / `try_field_mut` 严格寻址（无越界兜底，孤儿事件静默丢弃，防止经 `branch_name` 兜底污染无关表单）；**冲突结果区**固定为只读文档视图 + 按块接受/AI 回填（`conflict_result_pane_uses_editor` 开关与自绘编辑器死路径已删，`conflict_editor` 字段保留为焦点锚点）。多行普通 Enter 只换行，Ctrl/Cmd+Enter 由根层 `TextSubmit` 在 Kit 写入前提交。动态重绑快捷键只清除 Khaslana 自身 action，保留 Kit 的按钮、输入和菜单键位。项目级通用按钮使用 Kit 基础按钮；远端操作与标签推送的简单选择器使用 Kit `DropdownMenu`。真实前台键盘/焦点回归仍需按 M7 验收矩阵复验；向窗口注入按键要用 `PostMessage`，`SendMessage` 的同步重入会被 gpui 丢弃（WM_CHAR 两者皆可，WM_KEYDOWN 只有 `PostMessage` 生效）。


## 1. 项目定位

Khaslana 是一个使用 Rust 编写的桌面 Git 客户端，界面语言以中文为主。它基于 `gpui-pre` 与 `gpui-kit` 构建原生桌面 UI（M2 已完成底层切换，旧的 `gpui-ce` / `yororen_ui` 已移除），基于 `git2` / libgit2 执行 Git 操作，并通过系统 Keyring 保存 Git 凭据。
...
- UI：`gpui-pre =0.3.5`（Cargo 别名仍写作 `gpui`，故现有 `use gpui::…` 全部保留）+ `gpui-kit =0.6.4`（组件/资产/平台门面，与别名指向同一实例）。**版本必须精确锁 `=0.3.5`**：`^0.3.5` 会解析到 0.3.6，而 gpui-pre 的补丁版本之间有破坏性变更（0.3.6 与 gpui-component 0.6.4 的 inspector 签名不兼容，直接编译失败）
...
- 代码索引：`tree-sitter = "0.25"` 运行时 + 11 个语言语法 crate（rust/python/javascript/typescript(含 tsx)/go/java/c/cpp/c-sharp/php/kotlin-ng；官方 tree-sitter-kotlin 0.3.8 与新 runtime 的 links 声明冲突，故用 kotlin-ng）+ `ignore 0.4`（ripgrep 同源 gitignore 感知遍历），供 `src/code_index/` 引擎使用
...
- 主题桥接：`src/ui/kit_theme.rs` 把 Khaslana 语义 token 单向映射到 Kit 的 `ThemeColor`，再 `Theme::from(&colors)` 重建整个 `Theme`（Kit 的组件外观由 `ThemeColor` 一比一派生，只改零散字段不生效）
...
- `src/git/office.rs`：Office Open XML（docx/xlsx/pptx）文本提取纯函数层，SourceTree 式「Office 文档显示文本差异」的数据源。`office_text_lines(path, bytes)` 把 ZIP 内主文档解为文本行：docx 按 `word/document.xml` 的 `<w:p>` 段落（`<w:t>` run 拼接、`<w:tab/>` 制表符、`<w:br/>` 拆行、XML 实体含数字实体解码、空段落=空行）；xlsx 按 `xl/sharedStrings.xml` 字符串表 + 数字序遍历 `xl/worksheets/sheet*.xml`，每 `<row>` 一行、单元格制表符连接（`t="s"` 查表/`t="inlineStr"` 内联/其余取 `<v>` 原文）；pptx 按数字序遍历 `ppt/slides/slideN.xml`，每张先输出「── 幻灯片 N ──」分隔行再取 `<a:p>` 段落。手写 XML 轻量扫描（`scan_elements` 先定位开标签 `>` 再判自闭合——子元素如 `<w:tab/>` 的 `/>` 不能误当父元素结束；`is_tag_boundary` 排除 `<w:tabs>`/`<w:tc>` 同名前缀元素）。守卫：文件 ≤3MB（`office_file_diff` 在读 blob/工作区文件**之前**用 delta_side_size 的对象头/stat 尺寸先检后载，防止大文件先整体 inflate 进内存再被拒）、单条目解压 ≤8MB（`Read::take` 硬限，zip 头声明大小不可信）、zip 损坏/条目缺失/非法 UTF-8 → None 回退二进制占位卡。xlsx 按 `r="A1"` 列引用补空位（xlsx 省略空单元格，只连存在的单元格会让删除中间值后列错位；`<f>`-only 未计算公式记空串；无 r 属性退顺序追加）。`<w:cr/>` 与 `<w:br/>` 同拆行。接入点：`file_diff_from_diff` 顶部的 `office_file_diff` 钩子——提取双侧文本（old 侧查 ODB blob，new 侧 ODB 优先、id 为零读工作区文件）经 `Patch::from_buffers` 生成行级差异，文件头手工合成（真实路径 + 真实 blob 短 id，buffer diff 的哈希是内存内容哈希不可用），**`is_binary` 保持 true 仅额外填 lines**——全部二进制门控（拒绝部分暂存、隐藏全文/编码/追溯按钮、跳过语法高亮）原样生效，渲染层对「is_binary 且 lines 非空」显示文本行（占位卡条件从 `is_binary` 改为 `is_binary && lines.is_empty()`）；浏览模式 `browse_file_content` 同步提取。四个差异视图共用该钩子。旧版 OLE 格式（.doc/.xls/.ppt）不支持；AI 评审的 patch 文本仍按二进制占位（不静默改变 AI 输入）。
...
- `src/code_index/`：代码索引引擎（lib crate，Phase 1「代码智能」基础设施）。架构参照开源项目 codebase-memory-mcp（tree-sitter AST → 内存图缓冲 → SQLite 知识图谱）：**discover.rs** 文件发现——`ignore` crate 遍历（嵌套 .gitignore / info/exclude / 全局 excludesfile 全语义）+ ALWAYS_SKIP_DIRS 约 60 目录名剪枝（filter_entry 目录级整棵剪枝并计数）+ 二进制/生成物后缀黑名单 + 锁文件全名黑名单 + 子模块目录剪枝（`.git` 为文件）+ `MAX_INDEX_FILES`=50000 上限（对齐参考项目 auto_index_limit）；结果按 rel_path 排序保序。**lang_spec.rs** 12 语言规格表（Rust/Python/JS/TS/TSX/Go/Java/C/C++/C#/PHP/Kotlin，参照 lang_specs.c 的「节点类型字符串表 + 通用 walk」思路，不用 .scm query）：def/class_context/transparent/field/call/import/heritage/declarator_dig 各一张表；C 与 Python 的 function_definition 同名但语义不同，靠 `declarator_dig_types` 门控区分（仅 C/C++ 走 declarator 链挖函数名）。**extract.rs** 单遍 TreeCursor walk：作用域栈维护 class/impl 上下文（上下文内 Function 升级 Method）、fn_stack 记录调用点归属函数、透明包装下穿（export_statement/decorated_definition/template_declaration）、Go 方法 receiver 类型前缀（BFS 先找 type_identifier 再退化 identifier，防命中参数变量名）、JS 箭头函数经 variable_declarator 判定、匿名 struct/enum 跳过防兜底误吞字段名；导入提取 JS/TS 优先取 import 语句的 `source` 字段（整节点文本清洗会吞进 `{ x } from` 噪声），`clean_import_text` 剥前缀关键字/引号尖括号/`{...} from`/`.*` 通配/`::{..}` 名字段/相对路径段（`./`、`/`）；产出 owned 的 defs/imports/calls/type_refs。**graph.rs** 内存图缓冲（cbm_gbuf_t 同构）：nodes/edges Vec + qualified_name 哈希去重 + 边三元组去重，结构节点 upsert 幂等 / 符号节点同 QN 追加 `#N` 消歧（重载）；purge_files 级联删边后幸存节点 id 重排维持「id==下标」不变量（与 prune_orphan_modules 共用 rebuild_after_removal）；QN 规则 = `项目名.目录段.文件名[.类链.]符号名`（对齐参考 project.dir.parts.name）。**resolve.rs** 调用解析四级策略链（参考项目六级简化）：local(0.95) 同文件唯一 → import_map(0.90) 导入尾段匹配（导入段先剥 crate/self/super 根段再与文件尾段比较，`use crate::git::service` ↔ `src/git/service.rs`；整段不中再回退「去末段的容器路径」匹配——目录导入可达其下文件，对齐参考项目 is_import_reachable 的前缀可达语义）→ unique(0.80) 全仓库唯一 → suffix(0.60) 限定调用且排除本文件唯一；解析失败不建边，CALLS 边 properties 忠实记录 {callee, confidence, strategy}。**store.rs** SQLite 存储：每仓库独立库 `<数据目录>/code-index/<repo哈希8>/index.db`（repo 哈希复用 review_store 的 FNV-1a 小写折叠 repo_key，已提为 pub 共享），schema 与参考项目同构去 project 列（nodes/edges/file_hashes/meta + contentless FTS5）；主键不用 AUTOINCREMENT（整库重写后行号自然复用，增量载入 id 映射紧凑）；落盘走「删辅助索引 → 批量插入 → 重建」bulk 模式，FTS5 入库前 camel_split 驼峰拆分（免自定义 tokenizer，查询侧同样拆分 + 前缀 OR 匹配 + bm25 排序）；WAL 模式下 read_index_stats/search_symbols 只读连接与写入任务并发安全。**pipeline.rs** 编排：增量路由判定（库存在且 文件数 ≤ 已存哈希数 ×1.5 → 增量，否则全量，对齐 try_incremental_or_delete_db）；提取 pass 并行（std::thread::scope 连续分块，worker 数 = 核数-1 钳 [1,6]，每 worker 独立 Extractor/Parser，免锁保序）；增量 = mtime_ns+size 三分类（盘上仍在但未发现的模式跳过文件保守保留哈希）→ 整图载入 → **入边快照**（target 在变更文件而 source 幸存的跨文件边按 QN 捕获，防级联删除永久丢失）→ purge 重排 → 只解析 changed → 重链接快照（add_edge 去重幂等）→ **prune_orphan_modules**（清扫失去全部 IMPORTS 入边的 Module 节点——Module 无 file_path，purge_files 清不到，删导入语句后不扫会永久残留）→ 整库重写；取消标志在文件/阶段边界检查，取消不落盘（增量保留旧库）。守卫：>1MB 文件只登记 File 节点、前 8KB NUL 嗅探跳二进制、非 UTF-8 字节直解 + 名称 lossy 解码。Phase 1 明确不做（后续扩展点）：Embedding 向量（Phase 2 AI 搜索）、LSP 类型级解析、USAGE 引用边（schema 已支持）、git 耦合边、相似度边、路由节点。实测 khaslana 自身：183 文件 / ~5000 符号 / ~12800 边（CALLS ~7000，策略分布 local ~3160 / import_map ~1500 / unique ~2400 / suffix ~5）/ 2.9s。
- `src/code_index_view.rs`：设置中心「代码索引」分类页（bin crate，`SettingsCategory::CodeIndex`，导航图标复用 Search）。**多仓库列表形态**（设计稿 `design/code_index_settings.pen`）：打开设置页时 `refresh_code_index_stats` 构建条目列表（已打开 tabs → `load_recent_repos` 最近 20 → 索引偏好剩余键，三路合并按 repo_key 去重）。页面布局自上而下：页头 → MCP 服务器接入卡 → **仓库列表（页面底部）** → 引擎说明文案；无符号搜索验证卡（已删除，全局符号搜索走 Ctrl+P 面板）。列表随设置中心内容区整体滚动（不做内部虚拟滚动）；每仓库一张卡片——头行（`repo_avatar` 首字母色块 + 名称 + 状态徽标 + **Kit 滑动开关**（`toggle_switch` 包装的 `gpui_kit` 的 `Switch`，见开头「已确认决策」））+ 路径 + 状态行 + 按钮行，长文本 overflow_hidden + whitespace_nowrap 硬裁剪；状态四态纯函数 `code_index_entry_status`（Running > Indexed > DisabledWithData > NotIndexed，运行中优先、按「有数据+开关」分派）；运行中卡片主题色描边 + 比例进度条（`gpui::relative` 百分比宽度）+ 进度明细 + 「取消」，其余按钮禁用并注明「同一时间只能有一个索引任务」；已索引卡给 增量更新/重建索引/删除索引数据，已停用（关开关不删数据）给 重建索引/删除索引数据，未索引卡无按钮只留说明；列表标题行带过滤框（`FieldId::CodeIndexFilter`，名称/路径子串大小写不敏感，`code_index_entry_matches_filter`，打开设置页时重置）。开关与操作全部按显式仓库键寻址（`set_code_index_enabled_for` / `start_code_index_task_for_repo`，开关即时生效写 `code_index_preferences` 主库表 + 开启即发全量索引），不再限定活动仓库；统计打开设置页时对全部条目后台批量读库。**卡片内按钮/开关必须唯一 ElementId**：`app_button` 内部以 label 作元素 id，多卡同帧渲染同 label 按钮会共享元素状态（第一张卡可点、其余点击丢失），`code_index_interactive_host` 用「仓库键+动作」包一层有 id 的容器隔离状态路径（开关 id 同理带 repo_key）。任务编排：全局单任务守卫（`code_index_task: Option<CodeIndexTaskState>` 含 cancel AtomicBool）+ Index 池单线程双保险；`BackgroundTaskPanicked` 同样复位 code_index_task（全局单任务守卫 panic 后不复位会永久挡死索引入口与设置页状态卡）；自动触发两处——`RepositoryFastLoaded` 后（偏好启用时后台增量检查，无变化零写库零提示）与 `operation_refreshes_worktree_diff` 名单操作完成后（暂存/丢弃等改动工作区的操作后跟随刷新）；事件按 repo_path 键控（索引中关 tab 任务照常完成），Completed toast（完成即后台重读库统计覆盖缓存——任务内 stats 无库大小/时间/分支）+ Unchanged 仅状态栏文案 + Failed 错误 toast 且仅活动仓库提示；删除经 `DialogState::ConfirmDeleteCodeIndex { repo_key, display_name }` 确认弹窗（文案指明目标仓库名）后 remove_dir_all 整个 `<hash>` 目录（目录不存在视为已删除）。偏好缓存 `code_index_enabled_cache` 启动时与设置页打开时从主库加载。
...
- `src/credentials.rs`：凭据存储、匹配、Keyring 读写、凭据测试、旧存储兼容迁移和单元测试。`CredentialStore` trait 含 `update_secret`（按记录 id 原位替换 secret，Gitee 自动续期写回新令牌用）。`KeyringCredentialStore` 持 `index_lock: Mutex`——store 实例被多标签页凭据回调共享，所有 load→mutate→save_index 序列（save_record/touch_record/delete_record/update_secret/update_record_remote_url）必须持锁（save_credential_records 是全表 DELETE+重插，交错读改写会静默丢记录 + 孤儿密文）。host_key 携带协议（`http://host` 与 `https://host` 互不匹配，防 https 凭据明文重放给同域 http；旧记录统一 https 前缀不受影响）。`touch_record` 只更新 `last_used` 不覆写 `updated_at`（防列表按更新时间排序漂移）。Gitee OAuth 自动续期材料（refresh_token + expires_at）以 `GiteeRefreshPayload` JSON 存独立 Keyring 条目（`save/load/delete_gitee_refresh_payload`，服务名 `khaslana.git.credential:gitee-refresh:<record_id>`，与凭据本体分条目、不进 SQLite，载荷损坏读取时自清、记录删除时联动清理）。
- `src/ssh_credentials.rs`：本机 SSH 身份发现和凭据表单辅助，包括扫描 `~/.ssh` 私钥、解析 SSH config 的 `IdentityFile`、检测 SSH Agent 已加载身份和一键填入表单。
- `src/oauth.rs`：OAuth 快速登录服务层（GitHub Device Flow + Gitee 授权码流）。GitHub 走设备流（设备码请求、令牌轮询含 `authorization_pending`/`slow_down`/取消/过期、用令牌换取登录名；轮询 Agent 关闭 `http_status_as_error`，因 GitHub 待定/过期返回 400 且详情在响应体里）；Gitee 走授权码流（`gitee_run_code_flow`：本地 `127.0.0.1:17890` 回调服务器用 `std::net` 手写、循环读齐请求头、校验 Host 为本机回调地址、state 用 OS CSPRNG 128 位随机 → 收 code → POST 给 broker 换 token → 取登录名，登录名请求的 token 走 `Authorization` 头而非 URL query；返回 `GiteeLoginGrant` 携带 broker 透传的 refresh_token/expires_at 供自动续期）。Gitee 自动续期：`gitee_needs_refresh`（距过期 ≤ `GITEE_REFRESH_MARGIN_SECS`=2h 触发，含已过期）与 `gitee_refresh_via_broker`（POST `{refresh_token}` 给同一 broker 端点换 `GiteeRefreshedGrant`，刷新与换码共用 `BrokerTokenResponse` 解析与 `into_grant`（expires_in → 绝对 Unix 秒））。纯同步 `ureq 3`，复用全局代理设置，不引入异步运行时。令牌作为 `GitCredential::UserPass` 的 secret 复用现有 Keyring 存储与 git2 认证路径，无需改动凭据数据模型。客户端不含 Gitee `client_secret`（公开分发会泄露），token 交换由部署在边缘平台的 broker 代办（见独立仓库 [khaslana-broker](https://github.com/Neglecton/khaslana-broker) 的 `edge-functions/gitee.js`）。
- Gitee OAuth 令牌交换 broker：实现位于独立仓库 [khaslana-broker](https://github.com/Neglecton/khaslana-broker)（`edge-functions/gitee.js`，部署到腾讯云 EdgeOne 边缘函数或任意 serverless 平台），持有 `GITEE_CLIENT_SECRET` 环境变量，替客户端用授权码向 Gitee 换 access_token、或用 refresh_token 刷新（同一端点按请求体是否含 `refresh_token` 分流两种 grant_type，响应原样透传）。客户端只持 `GITEE_OAUTH_CLIENT_ID` + `GITEE_BROKER_URL`（`src/oauth.rs` 常量，二者均非空时 Gitee 登录按钮启用）。后续若新增其它 serverless 服务，也统一放进该仓库。
...
- `src/code_index/queries.rs`：索引查询 API 层（Phase 2 查询面，参照 codebase-memory-mcp 工具语义）。`find_symbol_candidates` 精确名/qualified_name 查找；`trace_calls` CALLS 边 BFS（起点=同名词根联合遍历，visited 防环，max_nodes=100；风险标签移植 cbm_hop_to_risk：hop 1→CRITICAL/2→HIGH/3→MEDIUM/≥4→LOW；同名可调用多候选返回 Ambiguous 让调用方用 qualified_name 重查）；`symbol_detail` 定义信息 + 一跳 callers/callees + 源码片段（rel_path 读文件钳 200 行/1MB）；`index_overview` label/边类型分布 + 扩展名语言聚合 + 顶层目录符号密度 + fan-in Top10 热点；`changed_files_via_git`（git2 实现，替代参考项目 shell git 三命令：默认 statuses 三态并集=未暂存+已暂存+未跟踪，传 base_branch 走 merge-base 三点 diff）；`impacted_symbols`/`impacted_symbols_for_files` 变更文件→文件内定义符号→CALLS 入边展开调用方（Phase 4 影响分析雏形）。图遍历基于 store.load_graph() 单次载全图（Phase 1 规模毫秒级；超大仓库优化点=SQL 邻接查询）；全部查询走 `open_read_only_if_exists` 只读打开（文件不存在/schema 版本不符 → 中文错误，**绝不创建幽灵空库**，对齐参考项目只读语义），trace/detail 单次载图内完成名称解析（无二次开库）。
- `src/code_index/mcp.rs`：内嵌 MCP stdio 服务器（lib crate，可单测），由 `main()` 顶部 `khaslana mcp [仓库路径]` 子命令分支调用（先于 GUI 启动；不初始化 stdout 版 tracing 防污染协议流）。**两种启动形态**（对齐参考项目「单服务器 + 项目注册表」多仓库语义）：`khaslana mcp <仓库路径>` 单仓库模式（启动即校验并后台保障索引，工具调用固定查该仓库，per-仓库各挂一条的高级用法，repo 参数被忽略）；`khaslana mcp` 多仓库模式（推荐，MCP 配置与仓库零耦合）——启动只解析数据目录，工具调用经可选 `repo` 参数解析目标（仓库绝对路径，或 `list_projects` 返回的 8 位 repo 键；路径存在但非 Git 仓库显式报错），未传时仅一个已索引仓库自动选中、多个报错列出清单让模型自纠错、零个引导 refresh_index 建；首次触达的仓库自动后台建索引（每仓库每服务器生命周期一次，ensured 集合去重）。协议逐条对照参考项目 mcp.c：换行分隔 JSON 帧 + Content-Length 兼容（10MB 上限）；initialize 协商 protocolVersion（2025-06-18/2025-03-26/2024-11-05，未知回最新）+ capabilities.tools + serverInfo；notifications 静默、ping→{}、未知 method -32601、解析失败 -32700（id 0）、字符串/数字 id 原样回显；tools/call 结果 = 单行紧凑 JSON text + structuredContent 双通道（非错误且可解析为对象时附加），业务错误 = isError:true + {error, hint} 自纠错提示。8 个工具：list_projects/search_symbols/get_symbol_detail/trace_path/get_architecture/detect_changes/index_status/refresh_index（除 list_projects 外都带可选 repo 参数）。单线程串行处理（无锁），单条消息处理包 catch_unwind（意外 panic 降级为该请求的 -32603 Internal error，不杀死服务器）；响应帧格式跟随请求（Content-Length 帧按同格式回）。索引保障走**后台线程**（对齐参考项目 maybe_auto_index——大仓库全量建索引不阻塞 MCP 客户端 initialize/工具超时）：无库/空库自动全量建、有库增量检查，进度一律 stderr，建立期间工具调用得到「索引尚未建立」错误 + 重试 hint；refresh_index 与后台任务经 `indexing_repo` 槽（Mutex<Option<repo键>>）互斥且身份精确——查询报错只在「正在建的就是当前仓库」时补 hint，多仓库模式不误伤其他仓库的报错。按 repo 键解析的上下文根目录读 meta `repo_path`（store/pipeline 落盘时写入，旧库为空则下次索引落盘补写；为空时 detect_changes/refresh_index 给出明确中文错误而非静默）。核心 `handle_message` 纯函数式，协议行为与多仓库解析有单测。schema 版本 2：parse_label 修复（旧库 File/Function 标签互换）后旧库自动删除重建。
- `src/code_palette_view.rs`：全局符号搜索面板（Ctrl+P，`ShortcutAction::OpenCodeSearch`，默认 ctrl-p 可在快捷键设置改）。settings_center 式 dialog_overlay 居中双栏面板（左 320px 结果列表 / 右详情）：输入即查（每次变更 `code_palette_search_seq`+1 → Short 求 search_symbols(50)，事件带 seq 守卫乱序丢弃）；选中行变化 → `code_palette_detail_seq` 守卫取 `symbol_detail`（直接 callers/callees + 源码片段）；↑↓ 切换/Enter=关闭并 `open_blame_file`（内置追溯视图，最接近「跳到代码」）/Esc 关闭——在面板层 `capture_key_down` 拦截（先于输入框 TextUp/TextDown/Enter 提交），§8 白名单已记录该例外；点击遮罩关闭（overlay on_mouse_down + 面板 stop_propagation）；底部操作行复用 追溯此文件/文件历史/复制绝对路径/打开所在目录 四个现有动作；面板关闭后输入内容保留（重开续用并立即按现值查询）；↑↓ 越过可见范围时选中行自动滚入视口（估行高 + 视口行数）。挂载层级在普通对话框之上、AI 思考弹窗之下，并纳入 `close_popups` 弹层家族（与其他弹层互斥）；入口监听用根壳层元素级 `.on_action`（区别于其他快捷键的 App::on_action——元素级回调带 Window 可聚焦输入框）。FieldId::CodePaletteSearch 必须注册进 DEDICATED_FIELDS（tests/main.rs 的 dedicated_fields_cover_all_field_ids 回归测试守护，漏注册 = 渲染即 panic）。
- `src/chrome_view.rs`：Focus Workbench 应用壳层。顶栏 60px：品牌（窄档只留标志）、仅仓库名的仓库下拉、Ctrl+P 搜索入口、中文命令组及 32×32 窗口按钮。最大化按钮必须走 `WindowControlArea::Max` 原生命中区以支持最大化/还原切换。Context Navigator 展开态含模式按钮、仓库分组与底部设置/收起，收起态为 48px 窄条；窄窗覆盖态不改写全局展开偏好。普通壳层按钮经项目级 Kit 基础按钮参与 Tab 导航并支持 Enter/Space；窗口控制仍走原生命中区。宽度策略读取逻辑像素，`LayoutBand::Narrow`（< 1120）将文字命令退化为图标 + tooltip，贮藏/子模块入口始终可达。
- `src/worktree_view.rs`：工作区 Review Canvas，把暂存/未暂存虚拟列表、差异画布与提交框组成主任务区；保留部分暂存、编码/二进制/大文件处理、差异缓存和操作完成后的刷新代际守卫。**单双列表按暂存状态切换**（M5，计划 §5）：`change_sections_layout` 的 `show_staged` 在 `staged_count > 0 || staged_loading` 时才为真，无暂存内容时不渲染「已暂存变更」分区、未暂存列表独占左列（加载期先占位防布局跳动，加载完仍为空则收起）；分区标题行与差异区标题行用分组底色 `WB_SECTION_HEADER` 而非贯穿 `border_b_1` 分割线；提交区是 `WB_COMMIT_BAR` + `RADIUS_MD` + 外边距 + `control_shadow()` 的独立抬起提交条，不再用 `border_t_1` 与上方硬切；diff 正文区用 `WB_DIFF_SURFACE` 保持平整底色，不随面板悬浮感加投影/圆角。
...
- `src/proxy_view.rs`：网络代理设置弹窗，包括模式切换、自定义代理输入、保存和测试代理入口。M3 起按新样板排布为三张 `settings_card`（代理模式分段选择器 / 自定义地址 / 连接测试 + 保存）；三个代理地址字段已通过 M4 字段适配层使用 Kit Input。
- `src/stash_view.rs`：贮藏完整工作流 UI，包括创建贮藏、查看贮藏文件、加载贮藏 diff、弹出确认（`ConfirmPopStash`，应用改动到工作区并从列表移除，与删除确认同款弹窗）和删除确认。
- `src/workflow_view.rs`：Runbook Studio 工作流 UI：左侧模板导航、运行配置、步骤时间线与按运行状态展开的控制台；模板目录扫描与 JSON5 解析在短任务池后台执行（`refresh_workflow_templates` 发起、`UiEvent::WorkflowTemplatesLoaded` 回传 `apply_workflow_templates` 应用，切换到工作流页与手动刷新都不在 UI 线程做文件系统 IO）；模板导航使用轻量下标模型 + 单一 `uniform_list`，只能在可视回调中创建行元素，不能把全部模板预构造成 `Vec<AnyElement>`。模板行根节点必须 `w_full`，使选中/悬停底色和命中区域填满虚拟列表槽位；uniform_list 行内文本（全部列表，不止模板行）**禁止 `truncate()`/`text_ellipsis`**，一律 `overflow_hidden + whitespace_nowrap` 硬裁剪——uniform_list 每帧以 MinContent 测量第 0 项，行内容塌到 min-content 宽度（约 0），带省略号的 nowrap 文本在该测量轮按坍缩宽度截断后会被 TextLayout 记忆化固化到绘制（模板名永远只显示「…」），须改用 `overflow_hidden + whitespace_nowrap` 硬裁剪（侧边栏各分组列表钉住标题后第 0 项同样是数据行，条目行文本已随之改用硬裁剪）；运行配置的滚动内容节点也必须 `w_full`，为内部百分比宽度输入框建立确定列宽，否则输入外壳会按内容收缩。模板单击加载、外部路径清理陈旧选中态、`WorkflowExecutor`、`TaskKind::Long` 与 per-tab 运行状态保持既有语义。
- `src/workflow_editor.rs`：工作流模板可视化创建/编辑器。**分步向导（v4，前端重构）**：弹窗重构为四步向导 `WORKFLOW_WIZARD_STEPS`（基本信息 → 操作步骤 → 变量设置 → 完成），当前步在 `WorkflowEditorState.wizard_step`，第 2 步的「添加步骤选择器」是独立视图态 `step_picker_open`。外壳：顶栏标题+副标题+「关闭」主按钮、步骤指示器 chip（当前步 PRIMARY 高亮）、底部导航（左「← 上一步」/ 中「下一步 →」末步换「保存模板」走原 `save_workflow_editor` / 右步骤计数），错误条固定在底部导航上方、任何一步可见。第 1 步：AI 助手软底卡（✨ 标题+说明+多行输入+右对齐生成按钮）、模板名称/保存文件名、干净工作树开关 + 右侧引导卡 `workflow_guide_card`。**第 2 步卡片化**：每步骤一张全宽卡（序号徽标+类型下拉（glass_menu deferred 锚定触发器）+↑↓✕ 微按钮），参数槽/布尔开关/守卫下拉全部内联卡内渲染（`ensure_workflow_editor_fields_inited` 因此初始化**全部**步骤的槽文本框）；卡底**绑定条**做变量引用分析——纯函数 `extract_template_var_refs` 扫 `${...}`（内建 git./run./date: 忽略）、`workflow_step_binding` 对照 inputs/vars 键与更早 filterBranches 的 `out.*` 输出、`workflow_binding_summary` 拼文案：未声明变量→warning 色条+「去定义」按钮（`workflow_editor_jump_to_vars` 跳第 3 步）、仅输出→success 色、有引用→primary soft、无→灰。**添加步骤选择器**：「+ 添加步骤」切到选择器视图（搜索框走新字段变体 `WorkflowEditorFieldId::PickerSearch`，`filter_workflow_step_kinds` 按中文名/op/说明大小写不敏感过滤；常用6/高级5 两列选项卡网格=图标+中文名+op+一句描述；「返回列表」退出；点选项经 `workflow_editor_add_step_of_kind` 加步并自动回列表）；预设卡片仍只在步骤为空时展示于本步。第 3 步（替代旧「高级」折叠区，`advanced_expanded` 字段删除）：inputs/vars 两组**变量卡片**（键徽标 chip + 「被步骤 N 引用」标签（`workflow_var_referenced_by_steps` 反查）+ 字段两列排布 + 必填开关 + ✕ 删除）。第 4 步：模板摘要卡（模板名/步骤数/输入变量/自定义变量/运行要求五行）+ 保存位置条 + JSON5 预览（`build_workflow_definition` 成功时 `serde_json::to_string_pretty`，缺必填显示占位提示）。**编辑模式（v2）**：模板列表行右键菜单「编辑此模板 / 复制为副本 / 删除模板...」（`WorkflowTemplateContextMenu`，按项目惯例登记 close_popups / any_popup_menu_open / mouse_down_inside_context_menu / 根层 capture 两处链）。删除经 `DialogState::ConfirmDeleteWorkflowTemplate` 确认弹窗（danger 按钮）后同步 `fs::remove_file`；若删除的是当前加载的工作流则同时清空详情区与选中态，再刷新模板列表。反映射纯函数 `workflow_editor_data_from_definition`（领域定义 → 编辑数据，11 种步骤逐字段回填、inputs 含 description）+ 注释检测 `workflow_content_has_comments`（跳过字符串字面量扫描 `//` 与 `/* */`，字符串内 `//` 误报只多一次确认，安全方向）；原文件含注释先弹 `DialogState::ConfirmWorkflowEditComments` 确认丢失风险（暂存经 `pending_workflow_edit`，确认后 `confirm_workflow_edit_comments` 进编辑器），副本不弹（原文件不动，强制 `-copy` 后缀另存）。保存：重名校验排除自身（否则未改名保存必误报）、写盘目标经 `workflow_editor_save_target` 纯函数决策——按**文件主干**（大小写不敏感）比较而非全名（`.jsonc` 原件也原地覆盖并保留原扩展名；按全名比较会让非 `.json5` 模板每次保存都误判为改名多出一个新文件），主干不同（改名）写新路径并删除旧文件（重命名语义，写成功后才删，删除失败仅记日志不阻断；改名同时把该模板的快捷键绑定经 `migrate_workflow_shortcut_binding` 迁移到新文件名，复制为副本不迁移）；保存成功后重定位模板目录并全量刷新列表；标题按 `editing_path` 动态显示「编辑/新建」。输入变量行支持 description 往返（`WorkflowInputPart::Description`，空→None）。**AI 生成/编辑（v3）**：弹窗内「AI 助手」区块——多行需求输入框（`FieldId::WorkflowEditor(AiDescription)`，多行判定挂 `is_multiline_field`）+「AI 生成」按钮（`ai_loading` 独立防重入，不借用全局 busy；未配置 AI 禁用并说明原因）。触发 `generate_workflow_template_with_ai`：`sync_from_fields` 后组 `workflow_template_prompts`（system 经 `include_str!` 嵌入 `docs/workflows.md` 全文作格式参考 + 严格输出约束；user = 需求描述，编辑模式附当前模板 JSON5），经公共 `start_ai_thinking_task` 执行器跑 Long 任务（思维链流式弹窗，见 §1 AI 辅助条目）→ `validate_generated_content`；成功发 `UiEvent::AiWorkflowTemplateGenerated`，失败走共用 `AiRequestFailed`（其分支调 `handle_ai_workflow_template_failed` 把错误写进弹窗内错误条并复位 loading）。完成事件 `handle_ai_workflow_template_generated` 调 `apply_ai_generated_to_editor_data`（剥代码围栏 → `parse_workflow_json5` → 反映射回填；保留 editing_path/file_name，file_name 为空时按 AI 名称 slug 推导，有 inputs/vars 时完成事件把向导跳到「变量设置」步；解析失败中文错误写错误条不关弹窗）→ 重建 `WorkflowEditorState` 刷新表单 + toast；用户检查后走既有「保存模板」链路，生成期间关弹窗则结果静默丢弃。分两层：纯数据层 `WorkflowEditorData`（可单测）持有字符串/布尔形态编辑数据，`build_workflow_definition`（逐项中文校验：步骤必填槽、inputs/vars 键非空/不重复/不撞 `git.`/`run.`/`date:` 保留前缀，最后复用 `validate_definition` 收口）、`workflow_editor_file_name`（非法字符/重名/后缀规范化）与 3 个内置预设（同步当前分支/新建功能分支并推送/合并分支并推送，预设仅在步骤为空时展示防误覆盖）都在这层；UI 状态层 `WorkflowEditorState` 包一层文本框（`TextFieldState` 需要 GPUI Context 构造）。步骤文本槽跨类型复用（checkout 切 merge 分支名保留）；文本框惰性创建统一前移到 `RepositoryView::render` 顶部的 `ensure_workflow_editor_fields_inited`（`field_mut` 无 cx 不能建框，Kit 输入宿主与 `ensure_kit_fields` 依赖字段已存在），`workflow_editor_field_ref_mut` 只寻址不创建，`workflow_editor_field_or_fallback` 自由函数隔离 `field_mut` 编辑器分支的两次可变借用。保存闭环不变：`sync_from_fields` -> 校验 -> `json5::to_string` 序列化 -> `parse_workflow_json5` 回读守卫 -> 写 `workflow_templates_dir()` -> `refresh_workflow_templates()` + 选中加载 + toast。步骤类型排序保持「常用 6 种在前、高级 5 种在后」。已删除随旧版单页 UI 移除的入口：`workflow_editor_add_step`（改为 `add_step_of_kind`）、`workflow_editor_select_step`、`workflow_editor_toggle_advanced`、`workflow_step_draft_summary`。
...
- `src/ui/`：前端设计系统适配层。`theme.rs` 定义主题感知语义 token；`components.rs` 封装项目级 UI helper，通用文字/图标/分段按钮以 Kit 基础按钮承载焦点、Tab 与 Enter/Space；`kit_theme.rs` 把 Khaslana token 映射到 Kit Theme。悬浮工作台使用 `WB_*` token、`floating_panel` / `navigator_panel` / `settings_card` 与统一阴影；表面色经 `rgb()` 时必须为不带 alpha 的 6 位值，透明色只用 `rgba()`。`fields.rs` 是 M4 Kit 输入适配层：`TextFieldState` 保持业务真值，Kit `InputState` / `TextareaState` 负责渲染、选区、IME 与剪贴板；`ensure_kit_fields` 自动覆盖 `DEDICATED_FIELDS` 中除 `ConflictEditor` 焦点锚点外的全部静态字段。M6 起工作流动态字段（`WorkflowInput` / `WorkflowEditor`）也迁入 Kit：它们不进静态注册表，宿主由 `reconcile_dynamic_kit_fields` 每帧按 `active_dynamic_field_ids` 枚举「多退少补」，`Change` / `PressEnter` 回调经 `try_field` / `try_field_mut` 严格寻址——动态字段被模板重建或弹窗关闭销毁后，残留事件直接丢弃（`field()` 的 `branch_name` 越界兜底会把孤儿输入写进无关表单，严格寻址是防污染的关键）。用户编辑经 `InputEvent::Change` 写回真值，程序回填下一帧推给 Kit；`focused_field` 必须先让位给 Kit 字段。多行 `PressEnter` 不提交，Ctrl/Cmd+Enter 由 Input 上的 `TextSubmit` 覆盖绑定在 Kit 插入换行前处理。
- `src/theme_view.rs`：应用外观设置 UI 和运行时主题切换逻辑，支持跟随系统、浅色和深色、主题色更换，并同步重建 Kit 全局主题（`ui/kit_theme.rs`，含聚焦边框跟随主题色）。M3 起页面本身也按新样板排布：三张 `settings_card`（显示模式分段选择器 / 主题色色块 / 当前显示）。
- `src/sidebar_view.rs`：Context Navigator 的仓库上下文 UI，包括本地分支、远端、远端分支、标签、贮藏和相关右键菜单；全部分组可折叠（本地分支是唯一默认展开的分组，折叠状态 per-repo 存于 `RepoTabState.sidebar_sections`；无贮藏时贮藏分组整体隐藏）。**钉住标题架构**：分组标题（与打开时的分支搜索框）钉在各分组滚动区外常驻不滚动，只有展开分组的条目列表各自内部滚动——条目再多标题也始终可见，后续分组标题依次固定在区域下方；每个展开分组各自一个 `uniform_list`（轻量索引模型 `SidebarNavItem` 只存快照下标，可视回调才建行，不得预建全部 `AnyElement`），高度策略：条目 ≤8 行按内容定高（`Content`，定高盒不加 flex_none、被压缩时列表内部滚动兜底，保证标题永不被推出视口），>8 行平分剩余空间（`Fill`）；每分组独立滚动 id（`sidebar_section_scroll_id`）经 `uniform_scroll_handle` 按 tab 键控，切换仓库后各分组滚动位置保留。**条目行名称文本必须 `overflow_hidden + whitespace_nowrap` 硬裁剪、禁止 `truncate()`**：各分组列表的第 0 项是数据行，uniform_list 以 MinContent 测量第 0 项宽度，truncate 的省略号会被坍缩宽度固化（workflow 模板列表同陷阱）。分支搜索常见 ASCII 路径使用无分配大小写匹配，非 ASCII 名称回退 Unicode 规范化。
...
- `src/diff_view.rs`：共享差异视图模块，工作区/历史/贮藏/浏览四个入口共用。含两类职责：**全文/紧凑切换**（切换按钮渲染、扇出重新加载、文件过大自动回退）与 **共享差异渲染**（`diff_section_header` / `render_virtual_diff` / `render_diff_row` / `render_encoding_dropdown` / `encoding_button` / `syntax_spans_for_diff`）。后者 M5 按重构计划 §5 从 `repository_ui.rs` **逐字节原样搬来**（数据模型、虚拟列表、宽度测量、语法高亮槽位、部分暂存交互全部未改），目的是让 `repository_ui.rs` 回到「与具体页面无关的通用 UI」；`render_column_splitter` 属通用分栏，仍留在 `repository_ui.rs`。
...
- `src/text_input.rs`：表单字段的业务真值容器（M7 起仅此职责）。`TextFieldState`（焦点句柄 + 占位符 + `TextEditState` 值/密文/光标）仍被全项目约两百处表单读写点直接访问，Kit 宿主每帧经 `ensure_kit_fields` 与它对齐；自绘输入元素、自绘 `text_*` action 与 `EntityInputHandler` 通路已删除，编辑/IME/选区/剪贴板全由 Kit `InputState` / `TextareaState` 承担。真值与渲染态两份状态不得合并（同步规则见 `src/ui/fields.rs` 模块文档）。
...
- 多仓库并存：`tabs`、`active_tab`、`RepoTabState`；仓库切换下拉（IDEA 式三区：功能区/打开项目/最近项目）替代标签页行。切换/打开/克隆仓库**保持当前所在主页面**（`inheritable_main_mode` 纯函数：主模式 Worktree/History/Workflow/CommitGraph 跟随切换带到目标 tab；专用模式绑定 per-repo 状态不继承，落回目标 tab 自身模式），不再强制跳到提交记录页
...
- 工作流快捷键绑定（`workflow_shortcut_bindings` 表，单行 JSON payload，模板文件名 → `{keystroke, background}` 映射；机器本地偏好，不写入可分享的模板 JSON5）。
...
- 切换/打开/克隆仓库后**保持在当前所在的主页面**（工作区/提交记录/工作流/提交图谱跟随切换；追溯/浏览/冲突等专用页面绑定具体仓库状态，切换后落回工作区），不再强制跳到提交记录页
- 多仓库并存（仓库切换下拉：置顶克隆/打开 + 搜索仓库 + 打开项目[活动置顶] + 最近项目，按最近时间排序，下拉项可关闭已打开仓库）。「搜索仓库」位于打开仓库下面，默认为按钮，点击展开为输入框 + 小叉（收起恢复按钮并取消过滤）；每输入/删除一个字符实时过滤打开/最近两区（名称或完整路径子串匹配，大小写不敏感，`filter_repo_switcher_sections`，区内名称命中排在仅路径命中之前）；无结果显示「没有匹配的仓库」占位。下拉菜单固定锚定在触发器按钮正下方（非鼠标位置），点击菜单外部或再次点击触发器按钮均关闭，复用根层 `capture_any_mouse_down` 的点击外部关闭机制；命中判定带 4px 边缘容差（`point_in_repo_switcher`），避免菜单左缘与侧边栏分栏分割线重合时，点在边框上的点击被误判为外部而关闭菜单并触发分割线拖拽（分割线的 `start_resize_column` 不再调用 `close_popups`，统一由根层捕获关闭）；弹层菜单（`any_popup_menu_open`：仓库切换下拉、各类右键菜单、编码菜单）打开期间全部分割线（列分割线与提交图列宽分割条）不响应鼠标、不显示拖拽光标（`column_splitter_accepts_mouse_events` 双参数门控），遮挡层打开也会中止进行中的拖拽。
- 自动保存和恢复会话
...
- 丢弃单个、选中或全部变更
- 查看工作区 diff；选中未跟踪文件（？）时差异区展示整份文件内容（`show_untracked_content` + `recurse_untracked_dirs`——后者不可省：pathspec 把新旧迭代器都限定到文件完整路径，索引侧对未跟踪路径产出为空，libgit2 对祖先目录条目无该 flag 不下钻且目录路径匹配不上 pathspec，整个子树被跳过成零 delta，表现为子目录未跟踪文件「没有可显示的文本差异」而根目录正常），但白底显示不标绿（SourceTree 式：`FileDiff.untracked` 标记 + 纯函数 `display_diff_line_kind` 把 Added 行映射为 Context 配色，仅影响显示，服务层行 kind 保持 Added、部分暂存守卫不受影响）
...
- Office 文档（docx/xlsx/pptx）文本化差异：SourceTree 式地把文档正文提取为文本行做两侧差异（docx 按段落、xlsx 按表格行单元格制表符连接、pptx 按幻灯片分隔），工作区/历史/贮藏/分支比较四个差异视图共用（见 `src/git/office.rs` 与 `office_file_diff` 钩子）；未跟踪 Office 文档整份内容按未跟踪语义白底展示。**is_binary 保持 true**——部分暂存（按块/按行按钮不显示且服务层拒绝）、「全文/编码/追溯」按钮隐藏、无语法高亮全部沿用二进制门控，提取文本只用于查看；提取失败（损坏/加密/超限/旧版 OLE .doc）回退二进制占位卡。浏览模式只读内容视图同样支持提取预览
- diff 头部可折叠
...
- 冲突工作台三栏（当前版本/结果区/传入版本）带语法高亮：块状态背景保留为行背景、语法色做前景，结果区随按块接受/AI 生成重算（后台补算 + seq 防乱序）；底部 base 折叠面板 v1 不高亮。**结果区固定为只读文档视图**（M6 删除自绘可编辑草稿路径：原 `conflict_result_pane_uses_editor()` 开关恒 false 使编辑器渲染与草稿回写均为不可达死代码；草稿修改只经按块接受按钮与 AI 回填写入 `view.draft`，`conflict_editor` TextFieldState 保留为焦点锚点——工作台两处点击经 `window.focus(&this.conflict_editor.focus, cx)` 把键盘交给工作台；`sync_conflict_editor_into_state` 留空操作并注释恢复可编辑结果区时的接回点）
...
- 远端列表、选择、添加、编辑、删除
- fetch、pull、push
- pull 对话框提供「用变基代替合并」滑块开关（Kit Switch），默认关闭，开启后执行 pull --rebase
...
- stash 列表（左键条目即查看贮藏）、创建、apply、pop（确认弹窗）、drop（确认弹窗）、文件列表和 diff 预览
- 工作流模板可视化创建与编辑：「新建」按钮打开**四步分步向导**（基本信息 → 操作步骤 → 变量设置 → 完成，见 `src/workflow_editor.rs`）；模板行右键「编辑此模板」（含注释的文件先弹确认告知保存会丢注释）/「复制为副本」（载入内容强制另存新文件名）/「绑定快捷键...」（录制键位 + 「后台执行」开关，见 §5.10 动态工作流快捷键）/「删除模板...」（确认弹窗，删除的是当前加载工作流时同时清空详情区）；**AI 助手**：弹窗内用中文描述想要的模板功能，AI 参考工作流使用文档生成/修改模板并直接回填表单（编辑模式基于当前内容修改），用户检查微调后即可。操作步骤页每步一张卡片：参数直接填在卡内，卡底绑定条实时标出 `${变量}` 引用情况（未声明变量标黄并提供「去定义」直达变量设置页）；「+ 添加步骤」弹出选择器（搜索 + 常用/高级分组选项卡）点选即加；变量设置页以变量卡片统一管理 inputs/vars 并标注被哪些步骤引用；完成页展示模板摘要与 JSON5 预览，「保存模板」经序列化回读守卫后写模板目录并刷新列表自动选中加载
...
- OAuth 快速登录：在添加 HTTPS 凭据时点击品牌矩形按钮（GitHub/Gitee 带文字 logo，按当前主题选浅/深变体），浏览器授权后自动取登录名并保存，无需手动录入 PAT。
  ...
  - Gitee：授权码流（`gitee_run_code_flow`：本地 `127.0.0.1:17890` 回调收 code → POST 给 broker 换 token → 取登录名）。Gitee 不支持 Device Flow/PKCE，公开客户端不能内置 `client_secret`，故 token 交换由部署在边缘平台的 broker 代办（独立仓库 [khaslana-broker](https://github.com/Neglecton/khaslana-broker) 的 `edge-functions/gitee.js`），客户端只持 `GITEE_OAUTH_CLIENT_ID` + `GITEE_BROKER_URL`（二者均非空时 Gitee 按钮启用）。scope `projects user_info`。**Gitee access_token 约 1 天过期（平台限制）**，已实现自动续期：登录成功时把 broker 响应里的 `refresh_token`/`expires_in` 以 `GiteeRefreshPayload` JSON 存入独立 Keyring 条目（服务名 `khaslana.git.credential:gitee-refresh:<record_id>`，与凭据本体分条目，不进 SQLite，随记录删除一并清理）；此后任意 git 网络操作解析凭据时惰性检查（`TabCredentialProvider::maybe_refresh_gitee_token`，仅 host=gitee.com 的 HTTPS UserPass 记录），距过期不足 2 小时（`GITEE_REFRESH_MARGIN_SECS`）即经 broker 发 `grant_type=refresh_token`（同一端点按请求体是否含 `refresh_token` 分流），新令牌经 `CredentialStore::update_secret` 写回凭据本体、轮换出的新 refresh_token 覆盖存储；刷新失败只提示并沿用旧令牌（refresh_token 约 14 天不用会失效，用户重新浏览器登录即重置）。刷新同步发生在凭据回调路径上，跟随当前代理设置。
- SSH key + passphrase
- 可使用 SSH agent
- 新增 SSH 凭据时自动检测 `~/.ssh`、SSH config 和 SSH Agent，可一键选择已有身份，也可通过文件框手动选择私钥
- 凭据保存到系统 Keyring
- 凭据记录管理、删除、测试连接；「测试连接」先弹地址确认弹窗（`DialogState::TestCredential`）：预填记录保存的远端地址、可修改后再测（典型：裸主机地址换成真实仓库地址——测试是对该 URL 发起真实 Git 连接，裸站点地址可能因服务器不发起认证而 404，不代表令牌无效）；校验链 `validate_credential_test_url`（纯函数）：非空 → 协议族（HTTPS 记录须 http(s)、SSH 须 SSH 地址）→ **HTTPS 记录同站点拦截**（host_key 一致才放行——令牌不通用，跨站点测试只会得到误导性认证失败；SSH 私钥主机无关不拦）；输入框 Enter 等效开始测试
...
- 左侧 9 分类导航：凭据管理、网络代理、AI 设置、合并工具、代码索引、外观、更新设置、快捷键、关于。
...
- 更新设置页：自动检查开关、「接收测试版（Beta）更新」开关（`update_preferences.include_beta`，默认关——开启后 `manifest_sources_for` 先取 CNB 根 `khaslana-update-beta.json` 再滑落正式清单）、已跳过版本、检测到新版本时页内常驻「发现新版本」卡片（版本/发布时间/包大小/版本说明滚动区 + 立即更新）；新版本弹窗的版本说明为多行可滚动。
...
### 5.9.1 代码索引（Phase 1 · Code Intelligence 基础设施）

- per-仓库开关：设置中心「代码索引」页针对当前活动仓库启用/关闭（`code_index_preferences` 主库表，repo_path 主键）。开启立即后台建全量索引；关闭停止自动索引但保留已建数据（另有「删除索引数据」按钮，确认弹窗后删除整个 `<数据目录>/code-index/<仓库哈希8>/` 目录）。
- 索引引擎参照开源项目 codebase-memory-mcp 的架构：gitignore 感知文件发现 → tree-sitter 解析提取（12 种语言的符号/导入/调用点）→ 内存图缓冲（qualified_name 去重）→ SQLite 知识图谱（nodes/edges/file_hashes + FTS5）。调用关系经四级策略链解析（local → import_map → unique → suffix），边上记录 confidence/strategy。详见 `src/code_index/` 模块文档。
- 自动触发时机：开启开关时全量建一次；打开已启用仓库时后台增量检查（mtime+size 无变化零写入）；改动工作区的 Git 操作（暂存/丢弃等，`operation_refreshes_worktree_diff` 名单）完成后跟随增量刷新。手动入口：设置页「增量更新」「重建索引」。
- 进度与反馈：全局单任务（Index 池单线程 + UI 守卫双保险），状态卡显示阶段与 n/m 进度并提供「取消」（取消不落盘，增量场景保留旧库）；完成 toast 汇报 文件/符号/关系 计数，无变化仅更新状态栏文案，失败错误 toast。事件按 repo_path 键控，索引中关闭仓库标签任务照常完成落盘。
- 符号搜索验证卡：输入符号名（支持驼峰拆分，如 `pushBranch` 可用 "push branch" 命中）FTS5 BM25 检索，结果列 label 徽标 + 名称 + file:line。这是 Phase 2「AI 代码搜索」的查询入口雏形。
- 全局符号搜索面板（Phase 2 第一步）：Ctrl+P 唤出，输入即查（seq 守卫防乱序）、↑↓ 切换、右栏实时显示选中符号的直接调用关系（callers/callees + 风险分级 chip）、Enter 打开内置追溯视图、底部可复用 文件历史/复制路径/打开目录 动作；Esc 或点击遮罩关闭。查询 API 层（`src/code_index/queries.rs`）同时支撑面板与 MCP 工具。
- MCP 服务器（Phase 2 第一步）：`khaslana mcp [仓库路径]` 以无头模式启动内嵌 MCP stdio 服务器（协议对齐 codebase-memory-mcp：initialize 协商/tools/list/tools/call、单行 JSON text + structuredContent 双通道、isError+hint 业务错误约定），向 Claude Code / Cursor / ZCode 等外部 AI 工具暴露 8 个查询工具（list_projects/search_symbols/get_symbol_detail/trace_path/get_architecture/detect_changes/index_status/refresh_index）。**多仓库模式**（推荐）：`khaslana mcp` 无参数启动，一条配置服务所有已索引仓库，工具按可选 repo 参数选择目标（仓库绝对路径或 list_projects 的 repo 键，单仓库时省略），首次触达自动后台建索引——MCP 进程与 GUI 相互独立，程序未开时 MCP 照常可用，关闭 GUI 不影响已建立的连接；`khaslana mcp <仓库路径>` 为固定单仓库的高级用法。设置中心「代码索引」页提供可一键复制的多仓库配置片段（exe 路径 + `mcp` 参数）。detect_changes 即 Phase 4「Git 变更影响分析」的查询雏形：工作区三态变更 → 受影响符号 → 上游调用方风险分级。
...
- **动态工作流快捷键**：任意工作流模板可绑定快捷键（数量无上限）。机制是**单一载荷 action** `RunWorkflowShortcut { file, background }`（`#[derive(Clone, PartialEq, gpui::Action)]` + `#[action(namespace = app_action, no_json)]`——gpui-ce 的 `KeyBinding` 携带 action 实例，动态模板不需要静态槽位）：每条绑定在 `register_all_key_bindings` 中先于静态快捷键注册（意外撞车时后注册的静态键胜出；每条经 `Keystroke::parse` 预检防 panic）。绑定入口在工作流页模板右键「绑定快捷键...」弹窗（录制复用根 `capture_key_down` + `skip_shortcuts` + `Esc 取消`机制，`recording_shortcut` 类型为 `ShortcutRecordingTarget::{App, Workflow}`；弹窗内含「后台执行」滑块开关（Kit Switch），**默认关闭**）；冲突检查走统一纯函数 `find_keystroke_conflict`（静态↔工作流双向 + 工作流↔工作流）。触发监听器在设置中心或任意对话框打开时忽略；触发链 `trigger_workflow_shortcut`：合并进行中/tab 无仓库守卫 → 解析模板（列表 file_name 大小写不敏感匹配，列表未加载回退模板目录探盘；不存在则自愈移除绑定）→ `load_workflow_file` **同步重读磁盘最新内容**（模板被修改后运行的一定是最新版本；解析失败报错不运行）→ 按 key 合并回填已填变量（`merge_workflow_input_values`，非空旧值保留）→ 纯函数 `workflow_shortcut_trigger_decision` 三分支：必填变量缺失（无论前后台一律跳工作流页提示填写）/ 默认跳页运行 / 开启后台则留在当前页（进度状态栏 + 完成 toast，事件按 tab 路由与视图无关）→ 复用 `run_workflow` 执行闭环保卫。**生命周期**：删除模板即移除绑定；编辑器改名保存经 `migrate_workflow_shortcut_binding` 迁移（复制为副本不迁）；模板列表刷新（`apply_workflow_templates`，仅 Ok 分支）剪枝失效绑定；加载时 `prune_workflow_shortcut_bindings` 防御剪枝（撞静态键/同键重复/坏键位丢弃）。持久化在 `workflow_shortcut_bindings` 表（单行 JSON：模板文件名 → `{keystroke, background}`，`background` serde 默认 false）；设置中心「快捷键」页有工作流分区展示与清除。模板行右缘显示键位 chip（后台执行的加「后台」标记）。
- 新增静态快捷键动作时：在 `ShortcutAction` 枚举加变体 + `action_id`/`label`/`default_keystroke` + `actions!` 宏加对应 GPUI action + `register_all_key_bindings` 加 match 分支 + `register_shortcut_listeners` 加监听器；新增动态（带数据）动作时改用载荷 action derive 路线（见工作流快捷键条目）。
...
- `src/tests/git/office.rs`：Office 文本提取纯函数单测（`zip::ZipWriter` 构造真实 docx/xlsx/pptx fixture）——docx 段落/多 run 拼接/制表符/换行拆行/预定义与数字实体解码/空段落、缺 document.xml 与损坏 zip 截断字节返回 None、xlsx 共享表/富文本拼接/内联与数字单元格/空行/sheet 数字序（sheet10 在 sheet9 后）、pptx 幻灯片数字序与分隔行；仓库级测试在 `src/tests/git.rs`（未跟踪 docx 提取行 + 真实行号 + is_binary 保持 true、修改 docx 双侧 +/-、损坏 docx 回退仅文件头、xlsx/pptx 未跟踪冒烟）。
...
- `src/tests/workflow_editor.rs`：模板创建器纯函数——定义构建校验（空步骤/必填槽缺失指明序号与槽名/inputs 与 vars 键空值保留字重复/空白 trim）、可选字段 None 省略、文件名校验（空名/非法字符/后缀剥离/大小写不敏感重名拒绝）、3 个预设生成合法定义且序列化回读 round-trip 一致、FeatureBranch 预设的 `${target}` 引用、步骤类型元数据一致性（常用在前/op_name 互逆）、切换类型槽保值、guardRemoteBranch/deleteBranches 默认值跟随领域约定。v2 编辑功能：注释检测（无注释/行/块/字符串内 `//` 不算/转义引号）、反映射 round-trip（11 变体 + description + vars 定义→编辑数据→定义恒等）、空名称往返、重名排除自身、保存目标决策（`.jsonc` 原件原地覆盖/大小写不敏感主干比较/改名删旧/新建相对名）。v3 AI 生成：`apply_ai_generated_to_editor_data`（合法 JSON5 回填恒等且保留 editing_path/file_name、代码围栏剥离 + file_name 空时按名称推导、非法 JSON 中文错误且原数据不污染、未知 op 被领域校验拒绝；事件层在有 inputs/vars 时把向导跳到变量设置步）；prompt 构造（文档关键章节嵌入、编辑模式含现有模板、空白需求形态）。v4 分步向导纯函数：`${...}` 引用提取（内建命名空间过滤/去重/未闭合安全）、步骤绑定分析（used/unknown/filterBranches 输出）、绑定条三段文案、变量被步骤引用反查（1 起始序号）、选择器搜索过滤（中文名/op/说明大小写不敏感、空词全量）。
...
- 应用壳层固定采用 Focus Workbench：**60px 顶栏**（品牌 + 仅仓库名的仓库下拉 + 全局搜索入口 + 刷新/获取/拉取/推送中文命令 + 32×32 窗口按钮；设置入口在导航底部而非顶栏）、Context Navigator 承载模式按钮与仓库引用分组（无独立 Activity Rail 列；展开=带文字模式按钮+分组列表+底部「设置 / 收起」，收起=48px 窄条「展开箭头 + 模式图标 + 设置」，任何页面常驻）。内容区四周 16px 留白，导航与页面各是一张悬浮面板（`navigator_panel` / `floating_panel`），面板间隙即分栏拖拽区（默认无线，悬停/拖拽才显示指示）。Navigator 展开状态为单一共享值（`ContextNavigatorPreferences`，存于 RepositoryView 非 per-tab），模式切换与仓库切换都不得改变展开/收起。壳层必须保留 MainMode 加载副作用、RepoTabState per-tab 状态、窗口原生命中区、仓库切换下拉锚定/关闭、overlay 顺序和 blocker 行为。
- 宽度策略必须保持为可测试的纯函数，并以 `Window::viewport_size()` 的逻辑像素宽度作为运行时输入；顶栏不设 overflow 收纳，窄档（< 1120）命令退化为图标 + tooltip，「贮藏」「子模块」在所有宽度常驻为图标按钮——任何宽度下每个命令都必须可达。Context Navigator 在窄窗以不改写停靠偏好的临时覆盖层展开，不能压坏顶栏或窗口控制区。
- 主界面、弹框和输入框外壳应优先复用 `src/ui/components.rs` 的项目级 helper，例如 `app_panel`、`page_header`、`command_group`、`empty_state`、`dialog_panel`、`dialog_overlay`、`input_frame`、`segmented_button`、`list_row_surface`、`status_pill`。平面列表行默认不应有完整边框或阴影；选中态使用淡背景和指示条。区块级空态走 `EmptyState` builder（`.detail()` / `.action()` / `.fill()`），列表内占位走 `panel_empty_row`（行文本需与标题同基线时用 `panel_empty_row_aligned`）；「没有仓库」与「工作区干净」是两种空态，分别设计文案与可执行入口，禁止共用一个空白占位。
...
- **键盘交互（Kit 标准）**：普通控件参与 Tab/Shift+Tab；按钮支持 Enter/Space；菜单支持方向键、Enter 与 Esc；可取消浮层支持 Esc 并恢复到稳定焦点。文本框保留编辑、IME 与业务提交语义：多行普通 Enter 换行，Ctrl/Cmd+Enter 提交；单行 Enter 提交所在表单。快捷键录制、Ctrl+P 面板、应用级快捷键及变更列表 Shift/Ctrl+点击多选保持原有业务守卫。`register_all_key_bindings` 动态刷新时必须保留 Kit 已注册的组件键位，只移除 `text_input::`（现仅存 `TextSubmit`）/ `app_action::` 命名空间的应用绑定。
...
- 自绘输入通路（自绘输入元素、16 个 `text_*` action、`EntityInputHandler` 实现）已随 M7 清理删除：`src/text_input.rs` 只剩 `TextFieldState` / `TextEditState` 业务真值容器（焦点句柄 + 占位符 + 值），全部输入框的渲染、光标、选区、IME 与剪贴板由 Kit `InputState` / `TextareaState` 承担（见 `src/ui/fields.rs`）。业务 view 不得再新增自绘输入或 `COLOR_*` 引用，一律使用 `ui::theme` 或 `src/ui/components.rs`。
