...
- 工作流模板可视化创建与编辑：「新建」按钮打开单页编辑器（预设卡片 / 常用步骤优先 / 高级折叠区，见 `src/workflow_editor.rs`）；模板行右键「编辑此模板」（含注释的文件先弹确认告知保存会丢注释）/「复制为副本」（载入内容强制另存新文件名）/「删除模板...」（确认弹窗，删除的是当前加载工作流时同时清空详情区）；**AI 助手**：弹窗内用中文描述想要的模板功能，AI 参考工作流使用文档生成/修改模板并直接回填表单（编辑模式基于当前内容修改），用户检查微调后点「保存模板」即可；保存经序列化回读守卫后写模板目录并刷新列表自动选中加载
