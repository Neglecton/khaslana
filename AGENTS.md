...
- **键盘白名单（用户要求，全应用唯一保留的键盘行为）**：① 设置中心-快捷键可配置的应用级快捷键（含录制流程本身）；② 文本框内的编辑按键（Ctrl+C/V/X/A、Shift+方向键等）与多行框 Enter 换行、单行框 Enter 提交所在表单；③ 变更列表 Shift/Ctrl+点击 的范围/多选。**例外（唯一）**：全局符号搜索面板（Ctrl+P，`src/code_palette_view.rs`）在其面板层 `capture_key_down` 拦截 ↑/↓（切换选中）、Enter（确认=打开追溯）、Esc（关闭）——该弹窗是文本框主导的检索面板，键盘导航是其核心交互；面板之外仍然零键盘导航。**除此之外其余键盘处理一律不得新增**：所有按钮纯鼠标交互（不设 track_focus/tab_index/tab_stop/on_key_down——gpui-ce 对聚焦元素在 Enter/Space 松开时合成点击、鼠标按下自动聚焦，剥夺可聚焦性是唯一根治手段）、无字母快捷键、无 ↑↓/Esc 导航、无 BrowseContent 之类非文本框的 Ctrl+C/A。
