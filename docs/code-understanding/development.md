...
- Pencil 节点 `bi8Au`（1440×900）是 T5 完成/来源态的结构基准。实现顺序固定为：40px 页面头 → 94px 唯一提问区 → 可滚动回答区；不得再增加底部追问框、第二套“全部来源”区或独立的大型调用上下游区。
- 页头左侧固定显示“代码理解 / 用源码回答业务问题”，右侧使用“索引就绪”和“基础分析”两个轻量徽标。既有索引设置、历史能力可挂在徽标交互上，不为此增加一排传统工具按钮。
- 完成回答区固定为：完成状态行 → 结论概述 → 横向业务步骤 → 数据读写表 → 并排的“调用入口 / 尚未确认”。调用上下游和来源仍保留为数据与跳转能力，但不重复渲染为额外大区块。
- 业务步骤使用等宽横向卡片和小号箭头连接；首步骤用主题浅色强调。连接类型与失败分支数量放在节标题摘要中，不在每个箭头下堆叠文字。
- 宽窗来源侧栏默认 360px，可由现有 splitter 调整到 280～480px；回答可用宽度不足 720px 时，源码替换回答主区并显示“返回回答”。侧栏结构固定为 40px 文件头、38px 路径条、36px 语义页签、27px 源码行以及 86px 复验说明区。
- 来源侧栏语义页签固定为“定义 / 实现 / 引用 / 调用方 / 被调用方”。基础分析只读源码始终可用；未接入 Java 增强时保留页签结构并以能力状态控制交互，不另建第二个源码视图。
- 问题框复用多行输入能力，但视觉上必须是一体化边框容器：左侧输入、右侧 32～34px 主按钮。Enter 发送、Shift+Enter 换行；IME composing 时 Enter 只确认候选，不发送。完成后保留原问题，可直接修改再提问；运行中问题只读，允许复制。
...
- 每次 T5 视觉改动必须以同一窗口尺寸、同一页面状态分别捕获 Pencil 和实际 GPUI 截图，逐项核对布局、间距、字体、颜色、图标、状态和交互；只看代码或空态截图不能宣称完成态与设计一致。结果写入仓库根目录 `design-qa.md`，P0/P1/P2 差异清零后才允许交接。
