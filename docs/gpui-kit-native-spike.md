...
- **再更正（2026-09-23，M3 缩放方案被推翻并重做）**：上面「发 `WM_NCLBUTTONDOWN` + 命中码交回系统模态缩放循环」这条**不成立**。实测 `SendMessageW(WM_NCLBUTTONDOWN, HTLEFT)` 到去边框窗口 0ms 返回、无模态循环、窗口尺寸不变——`DefWindowProc` 只为带 `WS_THICKFRAME` 的窗口起 size loop，命中码是手动发出的不算。同时「只去掉 `WS_CAPTION` 也不够」也只对了一半：客户区内缩可以用 `WM_NCCALCSIZE` 补偿，不必去掉样式位。**现行方案（Electron / tao / winit / egui 同款，实测通过）**：样式一个位都不动（`WS_THICKFRAME | WS_CAPTION` 全保留，系统显示后自动补 caption）；自绘圆角外观由 `chrome_view::shell_frame` 子类化补偿三件事——① 非最大化时 `WM_NCCALCSIZE` 把客户区 left/right/bottom 恢复成 proposed 窗口矩形（gpui 只恢复了 top；客户区铺满后非客户区面积为 0，DWM 无边框可画，M1 的「顶部白线」与「内缩 14 × 7」一并消失，白线来源是内缩出的环带而非 caption 样式位）；② `WM_NCHITTEST` 按系统 frame 厚度（`SM_CXSIZEFRAME + SM_CXPADDEDBORDER`，本机 4px）补左/右/下三边与两个下角的命中码——客户区铺满后 `DefWindowProc` 实测四边均只返回 `HTCLIENT`，ZCode 的 `HTLEFT` 也是 Electron 自己算的，不是 `DefWindowProc` 给的；③ 边框命中码的 `WM_NCLBUTTONDOWN`/`WM_NCLBUTTONDBLCLK` 直接放行 `DefWindowProc`，绕过 gpui 的元素树消费。顶边与两个上角仍归 gpui 的 `handle_hit_test_msg`（按 DPI 判 HTTOP 系列）。实测结果：样式 `thickframe=True caption=True`、客户区四边偏移 0、左/右/下/两角命中 `HTLEFT/HTRIGHT/HTBOTTOM/角`、带外 `HTCLIENT`、缩放循环随真实光标改变窗口尺寸、125% DPI 下最小窗精确 860 × 520（`border_offset` 重算归零）、最大化/还原按钮切换正常。自绘缩放带（`WindowResizeEdge` / `render_window_resize_bands`）已整体删除。**仍未真机目视**：圆角与边框线视觉、真实鼠标拖拽手感、DPI 动态切换。
