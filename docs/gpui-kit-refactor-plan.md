...
状态：首板原生样板已完成，M1 交互验证待补齐，主工程迁移尚未验收。更新日期：2026-09-21。
...
- 根据 M1 结果统一导入、宏与平台 API；Root 包住已有 RepositoryView。
...
截至 2026-09-21，根据原生样板记录：首板视觉、基础组件接入与关键业务依赖共同解析已完成；键盘策略已确认。M0 全量功能/性能基线与 M1 输入、弹层、滚动、DPI 等仍需补齐，尚未给出主工程迁移 Go 结论。共同解析通过不代表完整主工程编译通过。

1. **补齐 M1 并收口 M0 基线。** 在现有 `kit-spike` 验证中文 IME、剪贴板/撤销、真实输入、嵌套菜单/弹窗、标准键盘交互、10,000 行列表和 20,000 行 Diff、主题切换、窗口控制及 DPI；记录实际通过项、未通过项和 Go/No-Go 结论。
2. **通过后执行 M2。** 一个可构建边界统一运行时、Root、资产和主题；旧自绘页面先适配至同一运行时。验收既有主路径与 MCP 无头模式，完成 release 零错误零警告构建。
3. **M3 后优先完成 M5 的工作区部分。** 先完成新壳层及主题/代理设置的小范围适配验证，再接通真实仓库切换、变更列表、Diff、提交和提交并推送，形成可日常使用的纵向闭环。依赖的字段/弹层适配从 M4 前置完成。
4. **继续 M4、M5 历史部分和 M6。** 扩展通用表单、菜单、设置，再迁移历史及专业页面，最后执行 M7。每批保留截图、功能回归和未完成项，不以样板完成代替全量验收。

阶段编号沿用原计划，实际顺序按上述依赖调整；24–39 人日是初始总量估算，不代表当前剩余工期，M1 收口后按实际差异重估。
