...
不想手写 JSON5？在工作流页面点击模板栏的“新建”按钮，用表单式编辑器创建模板：

1. **从预设开始**（可选）：编辑器提供“同步当前分支”“新建功能分支并推送”“合并分支并推送”三个常用预设，点卡片一键载入后微调；也可以跳过直接添加步骤。
2. **填写基本信息**：模板名称（显示在列表中，可选）和保存文件名（必填）。
3. **添加步骤**：左侧点击“+ 添加步骤”，选中步骤后在右侧选择类型并填写参数。步骤类型下拉中常用的 6 种（切换分支、获取、拉取、新建分支、合并分支、推送）排在前面，其余 5 种高级类型带“高级 ·”前缀。每个参数都有中文说明和示例占位文本。
4. （可选）展开底部“高级选项”定义输入变量（运行前让用户填写）或自定义变量（固定值）。
5. 点击“保存模板”。Khaslana 会校验必填项、生成 JSON5 文件写入模板目录，并自动选中新模板——之后按下面的运行流程执行即可。

说明：

- 编辑器只支持**新建**；已有模板仍需手工编辑文件（编辑器重新保存会丢失手工注释）。
